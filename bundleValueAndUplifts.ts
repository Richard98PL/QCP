...
export function onAfterCalculate(quote, lines) {
  ...
  let bundleParents = findBundleParents(lines);
  bundleParents.forEach(function(bundleParent){
	  let children = findChildren(lines,bundleParent);
	  doTheUpliftDiscountsToTheBundle(children,bundleParent);
  })
  //uplifts
  fillBundleLevelValues(lines);
  return Promise.resolve();
};

function findBundleParents(lines){
  let bundleParents = [];
  if (lines.length) {
...
function markBundleParents(lines){
...
function findChildren(lines,parentOfFamily){
  let children = [];
  if (lines.length) {
	  lines.forEach(function (line) {
		  let parent = line.parentItem;
...
function fillBundleLevelValues(lines){
  if (lines.length) {
	  lines.forEach(function (line) {
